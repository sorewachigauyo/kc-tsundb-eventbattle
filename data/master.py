...
from data.static import SHIP_MASTER_FILENAME, EQUIP_MASTER_FILENAME, MST_SHIP_KEYS, MST_EQUIP_KEYS, WCTF_DB_FILENAME, ABYSSAL_SHIP_ID_CUTOFF, SHIP_TYPE
...
class ShipMaster:
    ...
    def is_carrier(self):
        return self.ship_type in [SHIP_TYPE.CV, SHIP_TYPE.CVB, SHIP_TYPE.CVL]
    
    def is_submarine(self):
        return self.ship_type in [SHIP_TYPE.SS, SHIP_TYPE.SSV]
...
def is_abyssal_ship(ship_id: int) -> bool:
...
for mst_ship in ship_master_raw:
    ...
    ship_master_table[ship_id] = ShipMaster(
        ...
        asw_base=wctf_db[str(ship_id)]["asw_base"] if not is_abyssal_ship(ship_id) else 0,
        asw_max=wctf_db[str(ship_id)]["asw_max"] if not is_abyssal_ship(ship_id) else 0,
        remodel_next_id=mst_ship[MST_SHIP_KEYS.REMODEL_NEXT_ID] if not is_abyssal_ship(ship_id) else 0,
        max_ammo=mst_ship[MST_SHIP_KEYS.MAX_AMMO] if not is_abyssal_ship(ship_id) else 0,
        max_fuel=mst_ship[MST_SHIP_KEYS.MAX_FUEL] if not is_abyssal_ship(ship_id) else 0,
        speed=mst_ship[MST_SHIP_KEYS.SPEED]
    )
