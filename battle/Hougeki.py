...
def calculate_special_attack_modifier(attack: HougekiAttack, attacker: PlayerShip, battle: Battle):
    ...
    if spattack < 7 or spattack in [HOUGEKI_CUTIN.ZUIUN_MULTI_ANGLE, HOUGEKI_CUTIN.AIR_SEA_MULTI_ANGLE]:
...
def calculate_anti_installation_precap(num: float, attacker: PlayerShip, defender: EnemyShip, is_hougeki=True):
    ...
    if dlc_count:
        dlc_lv = reduce((lambda x, y: x + max(attacker.stars[y[0]], 0) if y[1] > 0 and fetch_equip_master(y[1])["api_type"][2] == 24 else 0),
...
def apply_postcap_target_special_modifier(num: float, attacker: PlayerShip, defender: EnemyShip):
    ...
    # Supply Depot Princess
        ...
        if dlc_count:
            ...
            dlc_lv = reduce((lambda x, y: x + max(attacker.stars[y[0]], 0) if y[1] > 0 and fetch_equip_master(
